...
class Animal{
    // Classes have 2 major pieces
    // Fields -> properties associated with each object made from this class
...
class Dog extends Animal{
    ...
    // Overloading is the other half of polymorphism. This is when we have multiple instance of the same 
    // method but with a varying number of parameters (think methods with default arguments from wednesdays)
...
abstract class Car{
    // No constructor because we cannot make an instance of an abstract class. We expect this class to be
    // extended by some other class which is concrete (not abstract)

    // We can put in whatever methods we want later
    // In an abstract class we can have abstract methods (methods with no method body)
    abstract drive(): void;
}
...
let greenCar : Car = new Tesla() // Liskov Substitution Principle (abstract class = implementing class)
